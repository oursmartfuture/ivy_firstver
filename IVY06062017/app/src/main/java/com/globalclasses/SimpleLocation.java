 ...
 */
public class SimpleLocation extends Service {
     ...
     */
    private static final long INTERVAL_DEFAULT = 60 * 5 * 1000;
    /**
     * The factor for conversion from kilometers to meters
     */
    private static final float KILOMETER_TO_METER = 10000.0f;
    /**
     ...
     */
    private boolean mRequireNewLocation;
    /**
    ...
    public SimpleLocation(final Context context) {
        ...
        GlobalMethod.write("---simpleLocation.");
    }
    ...
    private LocationListener createLocationListener() {
        return new LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                ...
                GlobalMethod.write("onLocationChanged" + location);

                mPosition = location;
                cachePosition();
//				Toast.makeText(context, "onLocationChanged"+location, Toast.LENGTH_LONG).show();
                if (mListener != null) {
                    mListener.onPositionChanged();
                }

                if (location.getLatitude() != 0.0 && location.getLongitude() != 0.0) {
                    GlobalMethod.write("latitude====PK" + location.getLatitude() + " longitude=====PK" + location.getLongitude());
                    SharedPreferences sharePref = context.getSharedPreferences(Constant.pref_main, Context.MODE_PRIVATE);
                    SharedPreferences.Editor editor = sharePref.edit();
                    editor.putString("lat", location.getLatitude() + "");
                    editor.putString("lng", location.getLongitude() + "");
                    editor.commit();
                } else if (getLatitude() != 0 && getLongitude() != 0.0) {
                    GlobalMethod.write("latitude====" + getLatitude() + " longitude=====" + getLongitude());
                    SharedPreferences sharePref = context.getSharedPreferences(Constant.pref_main, Context.MODE_PRIVATE);
                    SharedPreferences.Editor editor = sharePref.edit();
                    editor.putString("lat", getLatitude() + "");
                    editor.putString("lng", getLongitude() + "");
                    editor.commit();
                }
            }
